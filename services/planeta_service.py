...
# POST
def criar_planeta(db: Session, planeta_in: schemas.PlanetaCreate) -> models.Planeta:
    # Regra: Se informou uma Estrela_id, ela deve existir no banco
    if planeta_in.estrela_id:
        estrela_existe = db.query(models.Estrela).filter(models.Estrela.id == planeta_in.estrela_id).first()
    ...
    novo_planeta = models.Planeta(**planeta_in.model_dump())
    db.add(novo_planeta)
    db.commit()
...
# GET
def listar_planetas(db: Session) -> list[models.Planeta]:
    return db.query(models.Planeta).all()

# PUT
def atualizar_planeta(db: Session, planeta_id: int, planeta_in: schemas.PlanetaCreate) -> models.Planeta:
    db_planeta = db.query(models.Planeta).filter(models.Planeta.id == planeta_id).first()
    if not db_planeta:
