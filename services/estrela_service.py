...
# PUT
def atualizar_estrela(db: Session, estrela_id: int, estrela_in: schemas.EstrelaCreate) -> entities.Estrela:
    # 1. Busca a estrela existente
    db_estrela = db.query(entities.Estrela).filter(entities.Estrela.id == estrela_id).first()
    if not db_estrela:
    ...
    nome_duplicado = db.query(entities.Estrela).filter(
        entities.Estrela.nome == estrela_in.nome, 
        entities.Estrela.id != estrela_id
    ).first()
    if nome_duplicado:
