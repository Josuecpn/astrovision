...
import schemas
import services.star_service as star_service
...
def modificar_estrela(estrela_id: int, estrela_in: schemas.EstrelaCreate, db: Session = Depends(get_db)):
    return star_service.atualizar_estrela(db, estrela_id, estrela_in)
...
def remover_estrela(estrela_id: int, db: Session = Depends(get_db)):
    star_service.deletar_estrela(db, estrela_id)
    return None
